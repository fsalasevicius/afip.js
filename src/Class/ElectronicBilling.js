...
module.exports = class ElectronicBilling extends AfipWebService {
  constructor(afip) {
    const RES = afip?.RES_FOLDER || path.resolve(__dirname, "..", "Afip_res"); // fallback

    const options = {
      soapV12: true,
      WSDL: path.resolve(RES, "wsfe-production.wsdl"),
      URL: "https://servicios1.afip.gov.ar/wsfev1/service.asmx",
      WSDL_TEST: path.resolve(RES, "wsfe.wsdl"),
  ...
  async createPDF() {
    throw new Error(
      "createPDF no está disponible en modo directo (sin proxy). Generá el PDF en tu app.",
    );
  }
  ...
  async createVoucher(data, returnResponse = false) {
    ...
    if (returnResponse === true) {
      ...
      if (Array.isArray(results.FeDetResp.FECAEDetResponse)) {
        results.FeDetResp.FECAEDetResponse =
          results.FeDetResp.FECAEDetResponse[0];
      }

      return {
        CAE: results.FeDetResp.FECAEDetResponse.CAE,
        CAEFchVto: this.formatDate(
          results.FeDetResp.FECAEDetResponse.CAEFchVto,
        ),
      };
    }
  }
  ...
  async getVoucherInfo(number, salesPoint, type) {
    ...
    const result = await this.executeRequest("FECompConsultar", req).catch(
      (err) => {
        if (err.code === 602) {
  ...
  async getSalesPoints() {
    return (await this.executeRequest("FEParamGetPtosVenta")).ResultGet
      .PtoVenta;
  }
  ...
  async getVoucherTypes() {
    return (await this.executeRequest("FEParamGetTiposCbte")).ResultGet
      .CbteTipo;
  }
  ...
  async getConceptTypes() {
    return (await this.executeRequest("FEParamGetTiposConcepto")).ResultGet
      .ConceptoTipo;
  }
  ...
  async getCurrenciesTypes() {
    return (await this.executeRequest("FEParamGetTiposMonedas")).ResultGet
      .Moneda;
  }
  ...
  async getOptionsTypes() {
    return (await this.executeRequest("FEParamGetTiposOpcional")).ResultGet
      .OpcionalTipo;
  }
   ...
   **/
  async getTaxTypes() {
    return (await this.executeRequest("FEParamGetTiposTributos")).ResultGet
      .TributoTipo;
  }
  ...
  async _checkErrors(operation, results) {
    ...
    if (operation === "FECAESolicitar" && res.FeDetResp) {
      ...
      if (
        res.FeDetResp.FECAEDetResponse.Observaciones &&
        res.FeDetResp.FECAEDetResponse.Resultado !== "A"
      ) {
